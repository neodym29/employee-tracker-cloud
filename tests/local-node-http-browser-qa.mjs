...
try{
 ...
 await page.getByRole('button',{name:'Scan repositories on my devices'}).click();await cli('once');await page.getByRole('button',{name:'Link to existing project',exact:true}).waitFor({timeout:20000});assert.equal((await db.query('select matched_project_id from tracemini_repository_candidates')).rows[0].matched_project_id,null);record('unmatched_link_visible',true);await page.screenshot({path:path.join(out,'unmatched-link-masked.png'),fullPage:true,mask:[page.locator('.command pre')]});await page.getByRole('button',{name:'Link to existing project',exact:true}).click();await page.getByRole('combobox',{name:/Existing project for/}).selectOption('700');await page.getByRole('button',{name:'Confirm link',exact:true}).click();await page.getByRole('link',{name:'Open existing project',exact:true}).waitFor();assert.equal(String((await db.query('select explicit_project_id from tracemini_repository_candidates')).rows[0].explicit_project_id),'700');record('completed_project_explicit_link',true);
