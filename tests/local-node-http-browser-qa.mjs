...
try{
 ...
 await page.goto(origin+'/projects');await page.getByRole('heading',{name:'Create project',exact:true}).waitFor();
 const summaries=page.locator('summary');for(let i=0;i<await summaries.count();i++){const text=await summaries.nth(i).innerText();if(/trace|CLI|device/i.test(text))await summaries.nth(i).click();}
 await page.getByRole('heading',{name:'Account repositories'}).waitFor();
 record('authenticated_next_projects',page.url().endsWith('/projects'));
 await page.getByRole('button',{name:/^(Prepare install command|Connect another device)$/}).click();await page.locator('.command pre').waitFor();
 ...
 await page.getByRole('button',{name:'Copy',exact:true}).click();assert.equal(await page.evaluate(()=>navigator.clipboard.readText()),command);record('original_install_copy_no_zip',true);
 await page.screenshot({path:path.join(out,'desktop-install-masked.png'),fullPage:true,mask:[page.locator('.command pre')]});
 for(const width of [1440,390]){await page.setViewportSize({width,height:1100});await page.screenshot({path:path.join(out,`${width}-install-masked.png`),fullPage:true,mask:[page.locator('.command pre')]});record('install_geometry_'+width,await page.evaluate(()=>({viewport:innerWidth,document:document.documentElement.scrollWidth})));}
 ...
 await page.getByRole('button',{name:'Scan repositories on my devices'}).click();await cli('once');await page.getByRole('switch').first().waitFor({timeout:20000});
