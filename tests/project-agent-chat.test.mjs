...
assert.match(service, /BACKEND_TIMEOUT_MS\s*=\s*180_000/, 'long project prompts need a model deadline above the former 45-second cutoff');
assert.match(service, /BACKEND_ATTEMPTS\s*=\s*2/, 'one transient backend retry is required');
assert.match(chatRoute, /maxDuration\s*=\s*300/, 'the production route must allow long model work');
...
assert.match(service, /Never ask for an upload or for the user to supply files/i);
assert.match(service, /Never require a file operation or document format/i);
assert.match(service, /process the available text/i);
