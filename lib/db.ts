...
async function ensureSchemaNow() {
  ...
  await db.query(`
    ...
    create table if not exists projects (
      ...
      constraint projects_pending_proposal_state_check check(coalesce(proposal_kind='engineer_client' and approval_status='pending' and status='draft',false) or approval_status<>'pending'),
      constraint projects_rejected_proposal_state_check check(coalesce(proposal_kind='engineer_client' and approval_status='rejected' and status='archived',false) or approval_status<>'rejected'),
      constraint projects_proposal_actor_check check(proposal_kind is null or creation_requested_by is not null)
    );
    create table if not exists project_memberships (
      ...
      is_project_proposal boolean not null default false,
      ...
      constraint project_memberships_proposal_shape_check check(not is_project_proposal or (membership_type='request' and created_by=user_id)),
      unique(project_id,user_id)
    );
    -- Intentionally unowned so generation values survive integration table lifecycle repair.
    ...
    alter table project_tracemini_integrations add column if not exists config_revision bigint not null default 1;
    alter table project_tracemini_integrations add column if not exists config_generation bigint default nextval('tracemini_integration_generation_seq');
    update project_tracemini_integrations set config_generation=nextval('tracemini_integration_generation_seq') where config_generation is null;
    alter table project_tracemini_integrations alter column config_generation set default nextval('tracemini_integration_generation_seq');
    alter table project_tracemini_integrations alter column config_generation set not null;
    ...
    create unique index if not exists idx_project_chat_messages_project_id_id_unique on project_chat_messages(project_id,id);
    ...
    create table if not exists project_agent_actions (
      ...
      confirmed_by bigint references app_users(id), confirmed_at timestamptz, result jsonb, display_description text, source_message_id bigint,
      created_at timestamptz not null default now()
    );
    ...
    create table if not exists project_tracemini_roots (
      ...
      approved_by bigint references app_users(id), approved_at timestamptz, created_at timestamptz not null default now(),
    ...
    create table if not exists project_tracemini_binding_codes (
      ...
      issued_by bigint not null references app_users(id), created_at timestamptz not null default now()
    );
    create table if not exists project_tracemini_bindings (
      id bigserial primary key, project_id bigint not null references projects(id) on delete cascade,
      device_id bigint not null references files_agent_devices(id) on delete cascade,
      binding_id_hash text not null unique check(binding_id_hash ~ '^[a-f0-9]{64}$'),
      binding_secret_hash text not null check(binding_secret_hash ~ '^[a-f0-9]{64}$'),
      root_hash text not null check(root_hash ~ '^[a-f0-9]{64}$'),
      status text not null default 'approved' check(status in ('approved','revoked')),
      created_at timestamptz not null default now(), last_heartbeat_at timestamptz,
      unique(project_id,device_id,root_hash)
    );
    create index if not exists idx_project_tracemini_bindings_device on project_tracemini_bindings(device_id,status);
    create table if not exists project_tracemini_events (
      ...
      root_hash text check(root_hash ~ '^[a-f0-9]{64}$'), event_key text not null,
      kind text not null check(kind in ('file_activity','non_git','dirty','commit','branch','merge','rewrite','pull','stage','push')), repository_key text,
      occurred_at timestamptz not null, provenance jsonb not null default '{}'::jsonb,
    ...
    create table if not exists project_tracemini_reports (
      ...
      next_run_at timestamptz, dedupe_key text, slack_status text,
    ...
    create table if not exists project_tracemini_schedules (
      ...
      include_diff boolean not null default false, document_consent jsonb not null default '[]'::jsonb,
      notify_slack boolean not null default false, next_run_at timestamptz, dedupe_key text unique,
      enabled boolean not null default true, created_at timestamptz not null default now(), updated_at timestamptz not null default now(),
      check(jsonb_typeof(selected_days)='array')
    );
    create table if not exists tracemini_runtime_settings(singleton boolean primary key default true check(singleton),tracemini_global_pause boolean not null default false,tracemini_embedded_enabled boolean not null default true,updated_at timestamptz not null default now());
    insert into tracemini_runtime_settings(singleton) values(true) on conflict do nothing;
    create table if not exists files_agent_rate_limits(scope_key text not null,window_start timestamptz not null,event_count integer not null check(event_count>=0),primary key(scope_key,window_start));
    alter table project_tracemini_roots add column if not exists binding_id text;
    alter table project_tracemini_roots add column if not exists binding_secret_hash text;
    alter table project_tracemini_roots add column if not exists repository_key text;
    alter table project_tracemini_roots add column if not exists last_heartbeat_at timestamptz;
    alter table project_tracemini_roots add column if not exists revoked_at timestamptz;
    create unique index if not exists idx_project_tracemini_roots_binding_id on project_tracemini_roots(binding_id) where binding_id is not null;
    alter table project_tracemini_events alter column root_hash drop not null;
    alter table project_tracemini_events add column if not exists root_id bigint references project_tracemini_roots(id) on delete cascade;
    alter table project_tracemini_events add column if not exists action text;
    alter table project_tracemini_events add column if not exists agent text;
    alter table project_tracemini_events add column if not exists run_id text;
    ...
    alter table project_tracemini_schedules add column if not exists last_run_at timestamptz;
    alter table projects add column if not exists tracemini_telemetry_paused boolean not null default true;
    alter table projects add column if not exists tracemini_retention_days integer not null default 90;
    alter table projects add column if not exists tracemini_resume_epoch bigint not null default 0;
    alter table project_tracemini_events add column if not exists resume_epoch bigint not null default 0;
    create table if not exists tracemini_audit_log(id bigserial primary key, project_id bigint not null references projects(id) on delete cascade, actor_user_id bigint not null references app_users(id), action text not null, details jsonb not null default '{}'::jsonb, created_at timestamptz not null default now());
    create table if not exists tracemini_runtime_settings (singleton boolean primary key default true check(singleton), tracemini_global_pause boolean not null default false, tracemini_embedded_enabled boolean not null default true, updated_at timestamptz not null default now());
    insert into tracemini_runtime_settings(singleton) values(true) on conflict do nothing;
    alter table project_tracemini_roots add column if not exists binding_id text;
    alter table project_tracemini_roots add column if not exists binding_secret_hash text;
    ...
    alter table project_tracemini_reports add column if not exists notify_slack boolean not null default false;
    ...
    begin
      ...
      if not exists (select 1 from pg_constraint where conname='projects_approval_status_check' and conrelid='projects'::regclass) then
        alter table projects add constraint projects_approval_status_check check(approval_status in ('pending','approved','rejected'));
      end if;
      if not exists (select 1 from pg_constraint where conname='projects_proposal_kind_check' and conrelid='projects'::regclass) then
        alter table projects add constraint projects_proposal_kind_check check(proposal_kind is null or proposal_kind='engineer_client');
      end if;
      if not exists (select 1 from pg_constraint where conname='projects_pending_proposal_state_check' and conrelid='projects'::regclass) then
        alter table projects add constraint projects_pending_proposal_state_check check(coalesce(proposal_kind='engineer_client' and approval_status='pending' and status='draft',false) or approval_status<>'pending');
      end if;
      if not exists (select 1 from pg_constraint where conname='projects_rejected_proposal_state_check' and conrelid='projects'::regclass) then
        alter table projects add constraint projects_rejected_proposal_state_check check(coalesce(proposal_kind='engineer_client' and approval_status='rejected' and status='archived',false) or approval_status<>'rejected');
      end if;
    ...
    begin
      if not exists (
        ...
        where m.project_id=new.project_id and m.id=new.source_message_id
          and m.role='user' and m.user_id=p.client_id
      ) then
        raise exception 'Client priority source must be the owning client user message in the same project' using errcode='23514';
      end if;
      return new;
    end $$;
    drop trigger if exists validate_project_client_request_summary_source_trigger on project_client_request_summaries;
    ...
    begin
      ...
      if not new.is_project_proposal then return new; end if;
      select * into proposal_project from public.projects where id=new.project_id for update;
      if not found then raise exception 'Project proposal has no project' using errcode='23503'; end if;
      ...
      if new.membership_status='active' then
        if proposal_project.approval_status='pending' and proposal_project.status='draft' then
          update public.projects set approval_status='approved',status='open',updated_at=now() where id=new.project_id;
        ...
        if proposal_project.approval_status='pending' and proposal_project.status='draft' then
          update public.projects set approval_status='rejected',status='archived',updated_at=now() where id=new.project_id;
        elsif proposal_project.approval_status<>'rejected' or proposal_project.status<>'archived' then
    ...
    create index if not exists idx_projects_approval_status on projects (approval_status,client_id,updated_at desc,id desc);
    ...
    create unique index if not exists project_memberships_one_proposal_per_project on project_memberships(project_id) where is_project_proposal;
  ...
  await db.query(`
    ...
    create index if not exists idx_project_client_request_summaries_project_created on project_client_request_summaries(project_id,created_at desc,id desc);
    create index if not exists idx_project_agent_actions_source_message on project_agent_actions(project_id,source_message_id) where source_message_id is not null;
  `);
  await db.query(`
