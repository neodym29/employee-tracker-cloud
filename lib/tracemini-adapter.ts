...
export class TraceMiniUpstreamError extends Error {
  constructor(message = 'Neo-Nexus transport is local-only', public transient = false, public code: TraceMiniErrorCode = 'unavailable') { super(message); }
}
...
export function validateTraceMiniBaseUrl(_value: unknown): never {
  throw new TraceMiniUpstreamError('Neo-Nexus does not accept a base URL');
}
...
export async function traceMiniGet(_baseUrl: string, _credential: string, _endpoint: TraceMiniEndpoint): Promise<never> {
  throw new TraceMiniUpstreamError('Neo-Nexus does not make upstream requests');
}
