  ...
  return (
    <div className="install-manual">
      ...
      <ol>
        ...
        <li><strong>Firefox has a different add-on flow.</strong> Firefox Release requires signed add-ons for permanent install. If Firefox says the add-on is <em>not verified</em>, do not open the XPI directly as a normal install. Instead open <code>about:debugging#/runtime/this-firefox</code>, click <strong>Load Temporary Add-on</strong>, and select <code>neodym-browser-firefox.xpi</code>. It will work until Firefox restarts. For permanent/silent Firefox installs, the XPI must be signed through Mozilla AMO or deployed as a signed enterprise add-on.</li>
      ...
      <table className="table browser-extension-manual">
        ...
        <tbody>
          ...
          <tr><td>Firefox / LibreWolf</td><td>Uses a Firefox-specific WebExtension XPI. Firefox Release blocks unsigned XPIs during normal install with a “not verified” warning, so use the temporary <code>about:debugging#/runtime/this-firefox</code> flow now and sign/deploy the XPI later for silent enterprise installs.</td><td>Download Firefox add-on XPI, open about:debugging, click Load Temporary Add-on, select neodym-browser-firefox.xpi, then verify Firefox rows on the dashboard.</td></tr>
      ...
      <ul>
        ...
        <li>If Firefox says the add-on is not verified: that means the XPI was opened as a normal install. Use <code>about:debugging#/runtime/this-firefox</code> → <strong>Load Temporary Add-on</strong> instead, or use Chrome/Brave/Edge until the Firefox XPI is signed.</li>
