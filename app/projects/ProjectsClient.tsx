...
export default function ProjectsClient({ accountType }: Props) {
  ...
  async function createProject(event: React.FormEvent) {
    ...
    const fingerprint = JSON.stringify(accountType === 'engineer'
      ? { accountType, clientId, title: title.trim(), description: description.trim(), gitRemote: gitRemote.trim() }
      : { accountType, title: title.trim(), description: description.trim(), gitRemote: gitRemote.trim(), status, engineerIds: sortedEngineerIds });
  ...
  return <div className="dashboardShell">
    ...
    {accountType === 'client' ? <>
      <div className="projectGrid projectTools"><section className="card"><h2>Create project</h2><form onSubmit={createProject} aria-busy={createBusy}><fieldset disabled={createBusy} className="formLock"><label>Title<input value={title} maxLength={120} onChange={(e) => setTitle(e.target.value)} required /></label><label>Description<textarea value={description} maxLength={4000} rows={4} onChange={(e) => setDescription(e.target.value)} /></label><label>Git remote<input value={gitRemote} maxLength={2048} placeholder="https://github.com/owner/repository.git" onChange={(e) => setGitRemote(e.target.value)} required /></label><label>Starting status<select value={status} onChange={(e) => setStatus(e.target.value)}><option value="open">Open</option><option value="draft">Draft</option></select></label><fieldset className="checkboxGroup"><legend>Add engineers (optional)</legend><p className="muted">Selected approved engineers join immediately as project creators with workspace access.</p>{engineers.length ? engineers.map((engineer) => <label className="checkboxLabel" key={engineer.id}><input type="checkbox" checked={selectedEngineerIds.includes(engineer.id)} onChange={() => toggleCreationEngineer(engineer.id)} />{engineer.display_name}</label>) : <p className="emptyLine">No approved engineers are available.</p>}</fieldset><button disabled={createBusy}>{createBusy ? 'Creating...' : 'Create project'}</button></fieldset></form></section>
